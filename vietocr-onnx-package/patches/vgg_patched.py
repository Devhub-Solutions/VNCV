...
class Vgg(nn.Module):
    ...
    def forward(self, x):
        ...
        # Dùng index dương để tương thích ONNX export (ONNX không hỗ trợ negative perm)
        conv = conv.transpose(3, 2)   # (B, D, H, W) -> (B, D, W, H)
        conv = conv.flatten(2)        # (B, D, W*H)
        conv = conv.permute(2, 0, 1)  # (W*H, B, D)
        return conv
